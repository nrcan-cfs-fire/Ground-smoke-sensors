...
ABHist <- read.csv("fp-historical-wildfire-data-2006-2024.csv")
...
NBAC <- read.csv("NBAC_2002_2024.csv")
...
Ground sensors detect particulate matter (PM) and volatile organic compound (VOC) concentrations, and report data every 5 minutes. PM and VOC sensors offer an advantage over CO and CH4 sensors via multi-channel data that allows for sensor diagnostics (i.e. concentration ratio between the size classes PM~2.5~ and PM~10~). Additionally, PM emissions are most closely related to both human health impacts [@matz2026] as well as fire suppression operations challenges related to reduced visibility due to wildfire smoke [@brambilla2025]. Sensors are powered by solar panels and communicate via LTE wireless protocol. Best deployed at a height of greater than 3 m (trees or utility poles). Warnings (first detection with a lower threshold) or Alerts (higher threshold) are sent out by the sensors if these concentrations pass a certain threshold determined by vendor-provided algorithms; warnings are approximated by PM~2.5~ observations exceeding 30 $\mu$g/m^3^ and alerts as observations exceeding 200 $\mu$g/m^3^. A web portal allowed stakeholders to monitor sensor status and receive text notifications for the wildfire ignitions Warnings and Alerts. The focus of this work is on the applied utility of the sensor network for small and localized smoke events; the precision or accuracy of lower-cost particulate concentration measurements has been extensively studied elsewhere [@holder2020]. The analyses contained herein are not meant to be an evaluation of the specifications and performance of the individual ground sensors. The main purpose of conducting these analyses is to provide advice to provincial/territorial wildfire suppression managers on what the most effective utilization of such ground sensors is operationally – either in conjunction with other detection methods, or on their own; and before, during and after a suppression operation.

## Other detection modes

Ground-based smoke sensor observations were compared to other more common detection modes, namely (1) visual detection (both public and fire agency) as well as (2) polar orbiting satellites for detection of smaller and smouldering fires, and (3) geostationary fire detection for large and fast-growing fires. Visual detection potential was estimated using two model used in conjunction: modelled fire growth and energy output from the Canadian Forest Fire Behaviour Prediction System [@forestrycanadafiredangerratinggroup1992], coupled to the widely used plume rise model from Briggs that can be applied to small to medium sized fires [@tory2018]. Since detailed fire growth records over time were not available, fire ignition records and hourly weather were used to drive the fire ignition until the final recorded area was achieved by the model. Fire growth modelling estimates with nearby weather observations are typically accurate within 20% of observations [@taylor2018], with similar accuracy in models of FRP and plume heights, resulting in a larger, compounded error rates of approximately 40% for those estimates. The relationship between plume rise height, diameter, and detection distance is not robustly understood; historical records indicate visual detection of forest fires \<1 ha is possible well over 25 km given clear line of sight and favourable atmospheric conditions (see Appendix F). Thus, any comparison to plume heights is merely for reference, as visual detection modes are very efficient in populated areas.

Satellite detection records (both polar orbiting and geostationary) for each individual fire event were obtained from the NASA FIRMS archive [@hewson2024]. A qualitative assessment of cloud cover for daytime periods was accessed via the NASA Worldview service for the satellite overpasses during fire events (including those without satellite detection). For each fire, the ground-reported ignition time (both wildfire and prescribed fire) was available with precision down to the minute from fire management records. The reported ignition time was then used as time zero each event, with fire growth and detection events calculated as minutes since ignition.

```{r load-process-VC-data-DO-NOT-RUN, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
main_path<-getwd()
subdir <- "VC"

full_path=file.path(main_path,subdir)

Alerts <- list.files(path=full_path, "Alerts*.csv$", recursive = T, full.names = T)
...
ACISHourlyData <- read_csv("ACISHourlyData-20240401-20240430-PID081045983.csv",col_types = cols(time = col_character()))
...
VC2023HourlyData <- read_csv("en_climate_hourly_QC_7016293_05-2023_P1H.csv",col_types = cols(time = col_character()))
VC2023HourlyData$time <- ymd_hm(VC2023HourlyData$time,tz="Etc/GMT+5") ##ECCC data is always LST

RC2024HourlyData <- read_csv("BCWS_394_Rock_Cr_hourly_gapfilled.csv",col_types = cols(time = col_character()))
...
WainwrightApr2024 <- read_csv("WainwrightApr2024.csv", 
...
VC2023FireReports <- read_csv("Valcartier2023.csv", 
col_types = cols(ReportedIgnTime = col_character(), 
FirstWarningTime = col_character(), 
FirstAlertTime = col_character(), 
RoundedIgnTime = col_character(), 
...
RC2024FireReports <- read_csv("RC2024.csv", 
col_types = cols(ReportedIgnTime = col_character(), 
FirstWarningTime = col_character(), 
FirstAlertTime = col_character(), 
RoundedIgnTime = col_character(), 
...
Sensor_Locs <- read.csv("Sensor_Locs.csv")
...
write.csv(FireGrowthTimeSeries,"CFB_Wainwright_FireGrowthTimeSeries.csv")
...
write.csv(FireGrowthTimeSeries_VC,"CFB_Valcartier_FireGrowthTimeSeries.csv")
...
write.csv(FireGrowthTimeSeries_RC,"RockCreek_FireGrowthTimeSeries.csv")
...
### do the +/- 20% or 40% error bats on AreaAtWarning, FRPAtWarning, and PlumeHatWarning as strings
#Sys.setlocale("LC_ALL","en_US.UTF-8")

### see link for discussion of math in kable table content
##https://stackoverflow.com/questions/66666775/how-to-include-arrows-and-superscripts-in-rmardown-table

###Area at warning, format for +/- 20%
all_sites <- all_sites %>% mutate(AreaAtWarning = paste0(round(AreaAtWarning,digits=-1), " $\\pm$ ",round(AreaAtWarning*0.2,digits=-1)))

all_sites$AreaAtWarning <- ifelse(str_detect(all_sites$AreaAtWarning,pattern="NA"),"",all_sites$AreaAtWarning)

all_sites$AreaAtWarning[10] <- paste0("0.3 ", "$\\pm$" ," 0.05")
#all_sites$AreaAtWarning
###end AreaAtWarning

###FRPAtWarning, format for +/- 40%
all_sites <- all_sites %>% mutate(FRPAtWarning = paste0(round(FRPAtWarning,digits=-1),  "$\\pm$" ,round(FRPAtWarning*0.4,digits=-1)))

all_sites$FRPAtWarning <- ifelse(str_detect(all_sites$FRPAtWarning,pattern="NA"),"",all_sites$FRPAtWarning)

all_sites$FRPAtWarning[10] <- paste0("2 ", "$\\pm$" ," 0.8")
#all_sites$FRPAtWarning
###end FRPAtWarning

###PlumeHatWarning, format for +/- 40%
all_sites <- all_sites %>% mutate(PlumeHatWarning = paste0(round(PlumeHatWarning,digits=-1), "$\\pm$" ,round(PlumeHatWarning*0.4,digits=-1)))

all_sites$PlumeHatWarning <- ifelse(str_detect(all_sites$PlumeHatWarning,pattern="NA"),"",all_sites$PlumeHatWarning)

#all_sites$PlumeHatWarning

###end FRPAtWarning

### round FinalArea and FRP

all_sites$ReportedFinalArea <- ifelse(all_sites$ReportedFinalArea > 10,as.character(round(all_sites$ReportedFinalArea,digits=0)),as.character(round(all_sites$ReportedFinalArea,digits=2)))

all_sites$FirstObsFRP <- round(all_sites$FirstObsFRP,digits=1)

all_sites$FirstObsFRP <- ifelse(all_sites$FirstObsFRP > 10,as.character(round(all_sites$FirstObsFRP,digits=0)),as.character(round(all_sites$FirstObsFRP,digits=1)))

##remove units for table printing
all_sites$DistToSensor <- as.vector(all_sites$DistToSensor)
...
kbl(all_sites_kbl,align="c",escape = F,caption="Fire ignition and detection characteristics of each fire event, in order of decreasing final fire size in each study area.  Area, Fire Radiative Power (FRP), and plume height at warning are all estimates based on fire growth modelling.") %>% kable_minimal() %>% column_spec(1:11, width = "1.8cm") %>%
 kable_styling(latex_options = "scale_down") %>%
 kable_styling(latex_options = "hold_position")
...
all_alerts <- read.csv("all_alerts.csv")

WR_PM <- read.csv("WR.csv")
...
knitr::include_graphics("plot-MW.png")

```

## Monitoring of sustained smoke production

In addition to the first detection, the smoke sensors produced useful information on the sustained production of ground-level smoke in the hours after ignition and growth of the prescribed and wildfires. Smoke detection events were registered at Wainwright up to 17 km from the fire area, at concentrations as high as 5,000 $\mu$g/m^3^ (Figure \ref{fig:WR_all_alerts}). Concentrations of PM~2.5~ have been registered as high as 20,000 $\mu$g/m^3^ at the immediate perimeter of fires by similar ground-based sensor packages by [@chwalek2023]. Smoke event observations the most distant from the fire were typically made after sunset through the early morning, and had a lower peak PM~2.5~ of less than 800 $\mu$g/m^3^. Afternoon smoke event observations made near the peak of the daily burning conditions (highest temperature and winds, lowest humidity) were confined to a distance of less than 2.5 km typically, and only in these observations within 2.5 km were PM~2.5~ concentrations in excess of 300 $\mu$g/m^3^ observed. Short duration and low concentration (\<50 $\mu$g/m^3^ smoke events) were only observed at distances of 5 km or less; smoke events observed closer were both of longer duration and higher concentration.

## Estimating visual detection range

At the time of the smoke sensor warning, the modelled plume top-heights where between 400 and 1,600 m above the ground. Against a high-contrast blue sky, such features are likely visible at a limit approaching the maximum aeronautical visibility range of 50--60 km, as supported by an analysis of detection distances by human observers from fire towers (see Figure A3). However, visual detection observations were not undertaken as a parallel effort during the smoke sensor deployment; future work on this topic would benefit from visual plume observation and detection in parallel to smoke and remote sensing. We do not explicitly model the visual detection of the smoke plumes, but rather provide this data as comparison against fire detection distances in nearby forest fire management zones with trained human observers.

Under widespread regional smoke or overcast conditions, the visual contrast between a smoke plume can diminish, and thus reduce the detection distance [@victorSimpleMethodPredicting1977]. Larger, more intense fires that are beyond suppression resources often feature darker (and larger) smoke columns [@stackInfluenceAirborneOcularly2024] which aids in the visual detection process for incidental ground-based observers. In Canada, fixed-wing dedicated fire detection [@foster1962; @mcfaydenRiskAssessmentWildland2019] uses human observers, and benefits from the visual contrast of typically light coloured smoke against a dark forest canopy background. Ideally, initial fire detections are made when the fire is a small sub-canopy surface fire, which reduces the ability of fixed-wing or satellite fire infrared detection due to canopy interception of infrared energy [@johnston2018].

## Regional smoke false positives from smoke sensors

In June of 2023, extensive regional smoke from wildfires 300--500 km upwind to the west in central Quebec severely impacted local particulate concentrations at the Valcartier deployment [@matz2025; @boulanger2024]. This far-field smoke source caused system warnings and alerts that were valid at the instrument level, but the smoke source was from very large and very well-observed fires that were being heavily suppressed. This smoke emission is well predicted in advance and in real-time using operational air quality forecasting systems that incorporate satellite fire detection and smoke dispersion schemes such as @chenFireWorkV20Air2019. These regional, widespread smoke emissions present a major source of false positive events in the context of local smoke and fire detection; furthermore, the detection of local fire events during periods of regional smoke remains a significant shortcoming in the fire detection event algorithm provided by sensor vendor.
