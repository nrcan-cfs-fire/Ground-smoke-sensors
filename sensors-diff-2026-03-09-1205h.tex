...
%DIF DEL C:\Users\thomp\Downloads\submitted.tex   Mon Mar  9 11:22:58 2026
%DIF ADD C:\Users\thomp\Downloads\revisions.tex   Mon Mar  9 12:04:17 2026
%  For support, please attach all files needed for compiling as well as the log file, and specify your operating system, LaTeX version, and LaTeX editor.

%=================================================================
% pandoc conditionals added to preserve backwards compatibility with previous versions of rticles

\documentclass[fire,article,submit,moreauthors]{mdpi}
...
Intense fires under light wind conditions can often lead to little to no
local smoke impacts due to efficient lofting of smoke within a large
fire convective plume; lighter fuels and stronger winds yields large
impacts to local air quality and even highway safety due to ground-level
smoke \citep{brambilla2025}. To date, distributed wireless small-sensor
networks of smoke and/or thermal detectors have not been operationally
deployed in Canada to use these ground-level smoke events as fire
detection systems. Such systems have been proposed in other
jurisdictions \citep[e.g.][]{sonDesignImplementationForestFires2006}
with primarily theoretical and simulation evaluation of such systems
\citep[e.g.][]{sernaDistributedForestFire2015}. Multi-sensor approaches
have been shown to be effective under moderate burning conditions in
grass fuels, with smoke particulate, organic gases, and thermal
detection approaches being the most effective \citep{chwalek2023}.
\DIFaddbegin 

\DIFaddend With the recent availability of commercial vendors making such
distributed small sensor systems available to communities or fire
management agencies, \DIFdelbegin \DIFdel{a}\DIFdelend n field evaluation of smoke-detecting ground
wildfire sensors was conducted through the 2023 and 2024 seasons during
prescribed burns as well as incidental wildfires at three locations in
varying terrain across Canada. \DIFaddbegin \DIFadd{This study represents the first large
field deployment and assessment of ground-based smoke detection sensors
across multiple prescribed and wildfires with a fixed network
(i.e.~sensors not redeployed around each fire). Thus, this deployment
simulates the case where dense sensor network deployment is used as part
of a fire detection and monitoring program. }\DIFaddend All fires consisted of
primary grass fuels with some shrubs and sparse trees. Ground-based fire
detection records were compared to a proxy for human visual detection
(smoke plume height) as well as satellite fire detection records. Data
on ignition time, fire weather, and final fire size were used in
conjunction with standard fire growth models to estimate fire size and
activity at varying detection intervals.
...
compound (VOC) concentrations, and report data every 5 minutes. \DIFaddbegin \DIFadd{PM and
VOC sensors offer an advantage over CO and CH4 sensors via multi-channel
data that allows for sensor diagnostics (i.e.~concentration ratio
between the size classes PM\textsubscript{2.5} and
PM\textsubscript{10}). Additionally, PM emissions are most closely
related to both human health impacts \mbox{%DIFAUXCMD
\citep{matz2026} }\hskip0pt%DIFAUXCMD
as well as fire
suppression operations challenges related to reduced visibility due to
wildfire smoke \mbox{%DIFAUXCMD
\citep{brambilla2025}}\hskip0pt%DIFAUXCMD
. }\DIFaddend Sensors are powered by solar
panels and communicate via LTE wireless protocol. Best deployed at a
height of greater than 3 m (trees or utility poles). Warnings (first
detection with a lower threshold) or Alerts (higher threshold) are sent
out by the sensors if these concentrations pass a certain threshold
determined by vendor-provided algorithms; warnings are approximated by
PM\textsubscript{2.5} observations exceeding 30
\DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} and alerts as observations exceeding 200
\DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3}. A web portal allowed stakeholders to
monitor sensor status and receive text notifications for the wildfire
ignitions Warnings and Alerts. The focus of this work is on the applied
utility of the sensor network for small and localized smoke events; the
precision or accuracy of lower-cost particulate concentration
measurements has been extensively studied elsewhere \citep{holder2020}.
The analyses contained herein are not meant to be an evaluation of the
specifications and performance of the individual ground sensors. The
main purpose of conducting these analyses is to provide advice to
provincial/territorial wildfire suppression managers on what the most
effective utilization of such ground sensors is operationally -- either
in conjunction with other detection methods, or on their own; and
before, during and after a suppression operation.
...
large and fast-growing fires. Visual detection potential was \DIFdelbegin \DIFdel{assessed
using the }\DIFdelend \DIFaddbegin \DIFadd{estimated
using two model used in conjunction: }\DIFaddend modelled fire growth \DIFaddbegin \DIFadd{and energy
output }\DIFaddend from the Canadian Forest Fire Behaviour Prediction System
\citep{forestrycanadafiredangerratinggroup1992}, coupled to the widely
used plume rise model from Briggs that can be applied to small to medium
sized fires \citep{tory2018}. \DIFaddbegin \DIFadd{Since detailed fire growth records over
time were not available, fire ignition records and hourly weather were
used to drive the fire ignition until the final recorded area was
achieved by the model. Fire growth modelling estimates with nearby
weather observations are typically accurate within 20\% of observations
\mbox{%DIFAUXCMD
\citep{taylor2018}}\hskip0pt%DIFAUXCMD
, with similar accuracy in models of FRP and plume
heights, resulting in a larger, compounded error rates of approximately
40\% for those estimates. }\DIFaddend The relationship between plume rise height,
diameter, and detection distance is not robustly understood; historical
records indicate visual detection of forest fires \textless1 ha is
possible \DIFdelbegin \DIFdel{upwards of }\DIFdelend \DIFaddbegin \DIFadd{well over }\DIFaddend 25 km given clear line of sight and favourable
atmospheric conditions (see Appendix F). \DIFdelbegin \DIFdel{Fire radiative power over time as a function of fire growth was also
estimated for each fire using the same fire growth models from
\mbox{%DIFAUXCMD
\citep{forestrycanadafiredangerratinggroup1992} }\hskip0pt%DIFAUXCMD
as were used in plume
...
is released as infrared radiation, following field-scale observations in similar fuels by \mbox{%DIFAUXCMD
\citep{johnstonDirectEstimationByrams2017}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{Thus, any comparison to plume
heights is merely for reference, as visual detection modes are very
efficient in populated areas}\DIFaddend .
...
detection). \DIFaddbegin \DIFadd{For each fire, the ground-reported ignition time (both
wildfire and prescribed fire) was available with precision down to the
minute from fire management records. The reported ignition time was then
used as time zero each event, with fire growth and detection events
calculated as minutes since ignition.
}\DIFaddend 
...
experienced a rapid increase from ambient \DIFdelbegin \DIFdel{PM2.5 }\DIFdelend \DIFaddbegin \DIFadd{PM\textsubscript{2.5}
}\DIFaddend concentrations of \textless10 \DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} to 3,260
\DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} at a sensor location 1.8 km downwind. PM
concentrations continued to be elevated above 100
\DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} for an additional 4 hours. GOES fire
detection occurred within 59 minutes after the completion of ignition
operations, with two GOES-18 hotspots totalling 107.7 MW detected. NOAA
VIIRS sensors detected a similar sum of 108 MW of fire radiative power
at 149 minutes after ignition during the early afternoon overpasses. All
of the additional sensors at Wainwright were deployed further south
except one sensor located 3 km to the west, which also did not register
any elevated PM concentrations. Photos of the strip backing fire
ignition pattern at 09:00 showed light winds corresponding to station
measurements of 10 km/h winds, but by the time of completion of ignition
operations at 11:00, winds had increased rapidly to 30 km/h and relative
humidity of 15\%, resulting in a Grassland Spread Index of 95 (Table
\ref{tab:allfires-ign-wx-table}). Of the large fires at Wainwright,
WR-14 had the most severe fire weather conditions of the large fires and
the second highest wind speeds at ignition of all Wainwright fires.
Unique to this fire was the rapid detection both by the smoke sensors as
well as the GOES geostationary satellite. This rapid spread rate coupled
with stronger winds leads to a lower plume height and more turbulent
mixing of smoke down to the surface
...
\DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} were detected in this dispersed
ground-level smoke plume.
...
excess of 200 \DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} were detected.
...
\begin{figure}
\includegraphics[width=0.95\linewidth]{../../DRDC-smoke/WR} \caption{\label{fig::WR-map}Wainwright study area showing fire, sensors, and dotted lines linking the first fire detection (warning or alert) for each detected fire.  Arrow indicates observed wind direction at the time of detection.}\label{fig:WR-map}
\end{figure}
...
%DIFDELCMD < \begin{tabular}[t]{>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{}p{1.8cm}|>{}p{1.8cm}|>{}p{1.8cm}}
%DIFDELCMD < \hline
%DIFDELCMD < FireName & Final area (ha) & Distance to sensor (m) & Time first warning (min) & Time first alert (min) & Time first VIIRS (min) & Time first GOES (min) & First Obs FRP (MW) & Est. Area at warning (ha) & Est. FRP at warning (MW) & Est. Plume height at warning (m)\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-15 & 600.00 & 6554 [m] & 246 & 355 & 766 & - & 2 & 600 & 2011 & 1367\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-14 & 550.00 & 1812 [m] & 13 & 23 & 149 & 59 & 108 & 7 & 117 & 629\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-12 & 375.00 & 529 [m] & 133 & 300 & 167 & 187 & 108 & 91 & 240 & 962\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-09 & 250.00 & 1192 [m] & 140 & 158 & 215 & - & 232 & 250 & 1232 & 1334\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-08 & 150.00 & 1739 [m] & 309 & 338 & 1095 & - & 2 & 150 & 445 & 780\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-16 & 75.00 & 4782 [m] & 396 & - & 218 & - & 7 & 75 & 147 & 935\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-01 & 1.00 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-04 & 1.00 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-06 & 0.60 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-07 & 0.28 & 4995 [m] & 86 & 112 & - & - & - & 0 & 2 & 230\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-13 & 0.25 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-11 & 0.20 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-05 & 0.05 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-02 & 0.03 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-03 & 0.02 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < WR-10 & 0.01 & - & - & - & - & - & - & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < VC-01 & 75.00 & 2109 [m] & - & 126 & 765 & - & 2 & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < VC-02 & 75.00 & 490 [m] & - & 202 & - & - & 2 & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < VC-03 & 6.00 & - & - & - & 163 & - & 3 & - & - & -\\
%DIFDELCMD < \hline
%DIFDELCMD < RC-01 & 75.00 & 891 [m] & 298 & 668 & - & 213 & 69 & 75 & 203 & 1058\\
%DIFDELCMD < \hline
%DIFDELCMD < \end{tabular}}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \resizebox{\ifdim\width>\linewidth\linewidth\else\width\fi}{!}{
\begin{tabular}[t]{>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}|>{\centering\arraybackslash}p{1.8cm}}
\hline
...
\hline
WR-15 & 600 & 6554 & 246 & 355 & 766 & - & 2.3 & 600 $\pm$ 120 & 2010$\pm$800 & 1370$\pm$550\\
\hline
WR-14 & 550 & 1812 & 13 & 23 & 149 & 59 & 108 & 10 $\pm$ 0 & 120$\pm$50 & 630$\pm$250\\
\hline
WR-12 & 375 & 529 & 133 & 300 & 167 & 187 & 108 & 90 $\pm$ 20 & 240$\pm$100 & 960$\pm$380\\
\hline
WR-09 & 250 & 1192 & 140 & 158 & 215 & - & 232 & 250 $\pm$ 50 & 1230$\pm$490 & 1330$\pm$530\\
\hline
WR-08 & 150 & 1739 & 309 & 338 & 1095 & - & 1.7 & 150 $\pm$ 30 & 440$\pm$180 & 780$\pm$310\\
\hline
WR-16 & 75 & 4782 & 396 & - & 218 & - & 6.9 & 80 $\pm$ 20 & 150$\pm$60 & 940$\pm$370\\
\hline
WR-01 & 1 & - & - & - & - & - & - &  &  & \\
\hline
WR-04 & 1 & - & - & - & - & - & - &  &  & \\
...
WR-06 & 0.6 & - & - & - & - & - & - &  &  & \\
\hline
WR-07 & 0.28 & 4995 & 86 & 112 & - & - & - & 0.3 $\pm$ 0.05 & 2 $\pm$ 0.8 & 230$\pm$90\\
\hline
WR-13 & 0.25 & - & - & - & - & - & - &  &  & \\
\hline
WR-11 & 0.2 & - & - & - & - & - & - &  &  & \\
...
WR-10 & 0.01 & - & - & - & - & - & - &  &  & \\
\hline
VC-01 & 75 & 2109 & - & 126 & 765 & - & 1.8 &  &  & \\
\hline
VC-02 & 75 & 490 & - & 202 & - & - & 1.8 &  &  & \\
\hline
VC-03 & 6 & - & - & - & 163 & - & 2.9 &  &  & \\
\hline
RC-01 & 75 & 891 & 298 & 668 & - & 213 & 69 & 80 $\pm$ 20 & 200$\pm$80 & 1060$\pm$420\\
\hline
\end{tabular}}
...
area, at concentrations as high as 5,000 \DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3}
(Figure \ref{fig:WR_all_alerts}). Concentrations of
PM\textsubscript{2.5} have been registered as high as 20,000
\DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} at the immediate perimeter of fires by
similar ground-based sensor packages by \citep{chwalek2023}. Smoke event
observations the most distant from the fire were typically made after
sunset through the early morning, and had a lower peak
PM\textsubscript{2.5} of less than 800 \DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3}.
Afternoon smoke event observations made near the peak of the daily
burning conditions (highest temperature and winds, lowest humidity) were
confined to a distance of less than 2.5 km typically, and only in these
observations within 2.5 km were PM\textsubscript{2.5} concentrations in
excess of 300 \DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} observed. Short duration and
low concentration (\textless50 \DIFdelbegin \DIFdel{ug}\DIFdelend \DIFaddbegin \DIFadd{\(\mu\)g}\DIFaddend /m\textsuperscript{3} smoke
events) were only observed at distances of 5 km or less; smoke events
observed closer were both of longer duration and higher concentration.
...
Figure A3). \DIFaddbegin \DIFadd{However, visual detection observations were not undertaken
as a parallel effort during the smoke sensor deployment; future work on
this topic would benefit from visual plume observation and detection in
parallel to smoke and remote sensing. We do not explicitly model the
visual detection of the smoke plumes, but rather provide this data as
comparison against fire detection distances in nearby forest fire
management zones with trained human observers.
}

\DIFaddend Under widespread regional smoke or overcast conditions, the visual
contrast between a smoke plume can diminish, and thus reduce the
detection distance \citep{victorSimpleMethodPredicting1977}. Larger,
more intense fires that are beyond suppression resources often feature
darker (and larger) smoke columns
\citep{stackInfluenceAirborneOcularly2024} which aids in the visual
detection process for incidental ground-based observers. In Canada,
fixed-wing dedicated fire detection
...
schemes such as \citet{chenFireWorkV20Air2019}. \DIFaddbegin \DIFadd{These regional,
widespread smoke emissions present a major source of false positive
events in the context of local smoke and fire detection; furthermore,
the detection of local fire events during periods of regional smoke
remains a significant shortcoming in the fire detection event algorithm
provided by sensor vendor.
}\DIFaddend 
...
sensors there, as well as the late Robert Prospero, Mark Batten,
Sgt\DIFaddbegin \DIFadd{.~}\DIFaddend Korosi and his team at the CF Detachment Wainwright for their
excellent support in the installation at that location. Finally, we
would like to thank Olivier Lundqvist, Luc Carrière and Christine
Bussières at SOPFEU; and Samuel Siddall, Neal McLoughlin, Andrew Simpson
and Ash Richardson and their colleagues at the BCWS for their support in
this initiative.
...
sensors from the vendor, and provided support for travel and sensor
deployment operations.}



\dataavailability{All the data required to reproduce the analysis, as
well as the RMarkdown build files, is available at
\url{https://doi.org/10.5281/zenodo.18202275}.}
